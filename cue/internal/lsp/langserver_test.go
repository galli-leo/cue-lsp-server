...
func TestServer(t *testing.T) { //nolint:funlen, gocognit, gocyclo
	...
	if hover == nil || !strings.Contains(hover.Contents.Value, "defined : {") {
		fmt.Println(hover)
	...
	if err != nil {
		t.Fatalf("Failed to apply full change to document: %v", err)
	}
	...
	if hover == nil || !strings.Contains(hover.Contents.Value, "value2 : defined") {
		...
		ContentChanges: []protocol.TextDocumentContentChangeEvent{
			{
				...
				RangeLength: float64(len(example_3)),
				Text:        example_3,
			},
		},
	})
	...
	doc, err := s.cache.GetDocument("test.cue")
	if err != nil {
	...
	if err != nil {
		t.Fatalf("Expected to be able to get diagnostics: %v", diagnostics)
	}
	if err == nil && len(diagnostics) == 0 {
		cont, _ := doc.GetContent()
		panic("expected nonempty diagnostics: " + cont)
	}
	...
	_, err = s.cache.GetDocument("test.cue")
	if err == nil {
